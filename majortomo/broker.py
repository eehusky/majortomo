...
class Broker:
    ...
    def _handle_client_message(self, message):
        ...
        if len(message.message) < 2:
            raise error.ProtocolError(
                "Client REQUEST message is expected to be at least 2 frames long, got {}".format(len(message.message))
            )
        ...
        self._log.debug(
            "Queueing client request from %d to %s", id_to_int(message.client), service_name.decode("ascii")
        )
    ...
    def _parse_incoming_message(frames):
        ...
        if len(frames) < 4:
            raise error.ProtocolError(
                "Unexpected message length: expecting at least 4 frames, got {}".format(len(frames))
            )
...
def main():
    ...
    def _parse_args():
        ...
        parser.add_argument(
            "-c", "--config", help="Configuration file path", type=argparse.FileType(mode="rb"), required=False
        )
        ...
        if args.heartbeat_timeout % args.heartbeat_interval != 0.0:
            raise parser.error(
                "--heartbeat-timeout ({}) must be a multiple of --heartbeat-interval ({})".format(
    ...
    broker = Broker(
        bind=args.bind_url or DEFAULT_BIND_URL,
