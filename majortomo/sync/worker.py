...
class Worker:
    ...
    def  wait_for_request(self)->tuple[bytes,list[bytes]]:
    ...
    def  send_reply_final(self, client:bytes, frames:Sequence[bytes])->None:
    ...
    def  send_reply_partial(self, client:bytes, frames:Sequence[bytes])->None:
    ...
    def  send_reply_from_iterable(self, client:bytes, frames_iter:Iterable[Sequence[bytes]], final:Sequence[bytes]|None=None)->None:
    ...
    def _receive(self)->tuple[bytes,list[bytes]]:
    ...
    def  _check_send_heartbeat(self)->None:
    ...
    def  _send_to_client(self, client:bytes, message_type:bytes, *frames:bytes)->None:
         self._send(message_type, client, b"", *frames)

    def  _send(self, message_type:bytes, *args:bytes)->None:
        assert self._socket is not None
...
def  main():
    ...
    with worker:
        ...
        while True:
            try:
                client, request =  worker.wait_for_request()
                worker.send_reply_final(client,request)
