...
class Client:
    ...
    def send(self, service: TextOrBytes, *args: bytes) -> None:
    ...
    def recv_part(self, timeout:float|None=None)->list[bytes]|None:
    ...
    def recv_all(self, timeout: float | None = None) -> Generator[list[bytes]]:
    ...
    def recv_all_as_list(self, timeout: float | None = None) -> list[bytes]:
    ...
    def request(self, service:bytes, *frames:bytes,timeout:float|None=None)->list[bytes]:
        self.send(service, *frames)
        return self.recv_all_as_list(timeout=timeout)
...
def main():
    ...
    with client:
        # client.send(b"worker1",b"Hello World")
        count = 0
        while count < 100:
            request = b"Hello world"
            try:
                response =  client.request(b"echo", request)
                print(response)
